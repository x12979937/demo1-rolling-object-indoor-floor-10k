...
def bind_texture_material(stage, prim, texture_path, material_path="/World/Materials/Demo1IndoorFloor"):
    if not texture_path:
        return False
    try:
...
def create_textured_floor_mesh(stage, UsdGeom, path, center, size, z, texture_path):
    ...
    bind_texture_material(stage, mesh.GetPrim(), texture_path, "/World/Materials/Demo1IndoorFloor")
    return mesh

def write_indoor_wall_texture(texture_path):
    texture_path = Path(texture_path)
    if texture_path.exists():
        return str(texture_path)
    texture_path.parent.mkdir(parents=True, exist_ok=True)
    try:
        ...
        yy, xx = np.mgrid[0:h, 0:w]
        base = np.zeros((h, w, 3), dtype=np.float32)
        base[..., 0] = 182 + 9.0 * np.sin(xx / 39.0) + 4.0 * np.sin((xx + yy) / 53.0)
        base[..., 1] = 188 + 7.0 * np.sin(xx / 41.0) + 4.0 * np.sin(yy / 47.0)
        base[..., 2] = 183 + 5.0 * np.sin((xx - yy) / 61.0)
        vertical_panel = ((xx % 128) < 4) | ((xx % 128) > 124)
        horizontal_trim = ((yy % 170) < 3)
        base[vertical_panel | horizontal_trim] *= 0.72
        img = np.clip(base, 0, 255).astype(np.uint8)
        cv2.imwrite(str(texture_path), cv2.cvtColor(img, cv2.COLOR_RGB2BGR))
        return str(texture_path)
    except Exception:
...
def create_textured_wall_mesh(stage, UsdGeom, path, center, size, texture_path, axis="back"):
    from pxr import Gf, Sdf, Vt

    cx, cy, cz = [float(x) for x in center]
    sw, sh = float(size[0]) * 0.5, float(size[1]) * 0.5
    mesh = UsdGeom.Mesh.Define(stage, path)
    if axis == "x":
        pts = [
            Gf.Vec3f(cx, cy - sw, cz - sh),
            Gf.Vec3f(cx, cy + sw, cz - sh),
            Gf.Vec3f(cx, cy + sw, cz + sh),
            Gf.Vec3f(cx, cy - sw, cz + sh),
        ]
    ...
    mesh.CreateDisplayColorAttr([Gf.Vec3f(0.72, 0.74, 0.72)])
    st = UsdGeom.PrimvarsAPI(mesh.GetPrim()).CreatePrimvar("st", Sdf.ValueTypeNames.TexCoord2fArray, UsdGeom.Tokens.varying)
    st.Set(Vt.Vec2fArray([Gf.Vec2f(0.0, 0.0), Gf.Vec2f(6.0, 0.0), Gf.Vec2f(6.0, 2.0), Gf.Vec2f(0.0, 2.0)]))
    bind_texture_material(stage, mesh.GetPrim(), texture_path, "/World/Materials/Demo1IndoorWall")
    return mesh
...
def setup_falling_floor_world(World, FixedCuboid, PhysicsMaterial, UsdGeom, UsdLux, floor_z=0.0):
    ...
    floor_mat = PhysicsMaterial(
        ...
        dynamic_friction=0.07,
        restitution=0.08,
    )
    floor_center = np.array([0.62, 0.0, floor_z - 0.010])
    floor_size = np.array([7.00, 4.50, 0.020], dtype=np.float64)
    FixedCuboid(
        "/World/FloorCollider",
    ...
    texture_path = write_indoor_floor_texture(
        "/root/autodl-tmp/mingyu/IsaacLab/Projects/ThreeDemo/assets/textures/demo1_indoor_room_floor_laminate.png"
    )
    create_textured_floor_mesh(stage, UsdGeom, "/World/IndoorFloorTexture", [0.62, 0.0], [7.00, 4.50], floor_z + 0.0015, texture_path)
    wall_texture_path = write_indoor_wall_texture(
        "/root/autodl-tmp/mingyu/IsaacLab/Projects/ThreeDemo/assets/textures/demo1_indoor_room_wallpaper.png"
    )
    _visual_prim(stage, UsdGeom, "/World/InteriorBackWall", "cube", [0.73, 0.75, 0.72], translate=(0.62, 2.25, 0.82), scale=(7.00, 0.035, 1.64))
    _visual_prim(stage, UsdGeom, "/World/InteriorLeftWall", "cube", [0.69, 0.72, 0.70], translate=(-2.88, 0.0, 0.82), scale=(0.035, 4.50, 1.64))
    _visual_prim(stage, UsdGeom, "/World/InteriorRightWall", "cube", [0.69, 0.72, 0.70], translate=(4.12, 0.0, 0.82), scale=(0.035, 4.50, 1.64))
    create_textured_wall_mesh(stage, UsdGeom, "/World/IndoorBackWallTexture", (0.62, 2.208, 0.82), (7.00, 1.64), wall_texture_path, axis="back")
    create_textured_wall_mesh(stage, UsdGeom, "/World/IndoorLeftWallTexture", (-2.838, 0.0, 0.82), (4.50, 1.64), wall_texture_path, axis="x")
    create_textured_wall_mesh(stage, UsdGeom, "/World/IndoorRightWallTexture", (4.078, 0.0, 0.82), (4.50, 1.64), wall_texture_path, axis="x")
    _visual_prim(stage, UsdGeom, "/World/BackBaseboard", "cube", [0.42, 0.36, 0.28], translate=(0.62, 2.205, 0.055), scale=(6.96, 0.030, 0.070))
    _visual_prim(stage, UsdGeom, "/World/LeftBaseboard", "cube", [0.42, 0.36, 0.28], translate=(-2.835, 0.0, 0.055), scale=(0.030, 4.40, 0.070))
    _visual_prim(stage, UsdGeom, "/World/RightBaseboard", "cube", [0.42, 0.36, 0.28], translate=(4.075, 0.0, 0.055), scale=(0.030, 4.40, 0.070))
    key = UsdLux.DistantLight.Define(stage, "/World/KeyLight")
...
def make_box_mesh(size, center=(0.0, 0.0, 0.0)):
    sx, sy, sz = [float(x) for x in size]
    cx, cy, cz = [float(x) for x in center]
    xs = [cx - sx * 0.5, cx + sx * 0.5]
    ys = [cy - sy * 0.5, cy + sy * 0.5]
...
def extrude_polygon_mesh(points_xy, height):
    pts = np.asarray(points_xy, dtype=np.float64)
    h = float(height) * 0.5
    n = int(len(pts))
    vertices = np.vstack([np.c_[pts, np.full(n, -h)], np.c_[pts, np.full(n, h)]])
    # Triangulation for the six-vertex L polygon used below.
    top_tris = np.array([[0, 1, 2], [0, 2, 3], [0, 3, 5], [3, 4, 5]], dtype=np.int64)
    faces = []
    for tri in top_tris:
...
def make_l_block_mesh(size):
    sx, sy, sz = [float(x) for x in size]
    t = max(min(sx, sy) * 0.42, 0.018)
    points = [
        [-sx * 0.5, -sy * 0.5],
        [sx * 0.5, -sy * 0.5],
        [sx * 0.5, -sy * 0.5 + t],
    ...
    return extrude_polygon_mesh(points, sz)


def make_cylinder_mesh(size, segments=40):
    sx, sy, sz = [float(x) for x in size]
    rx, ry, h = sx * 0.5, sy * 0.5, sz * 0.5
    angles = np.linspace(0.0, 2.0 * math.pi, int(segments), endpoint=False)
    bottom = np.array([[rx * math.cos(a), ry * math.sin(a), -h] for a in angles], dtype=np.float64)
    top = np.array([[rx * math.cos(a), ry * math.sin(a), h] for a in angles], dtype=np.float64)
    vertices = np.vstack([bottom, top, [[0.0, 0.0, -h], [0.0, 0.0, h]]])
    ...
    for i in range(n):
        j = (i + 1) % n
        faces.append([bc, j, i])
        faces.append([tc, i + n, j + n])
        faces.append([i, j, j + n])
        faces.append([i, j + n, i + n])
    return vertices, np.asarray(faces, dtype=np.int64)


def make_tetrahedron_mesh(size):
    sx, sy, sz = [float(x) for x in size]
    vertices = np.array(
        [
            [0.50 * sx, -0.30 * sy, -0.50 * sz],
            [-0.50 * sx, -0.30 * sy, -0.50 * sz],
            [0.0, 0.58 * sy, -0.50 * sz],
            [0.0, 0.0, 0.50 * sz],
        ],
...
def procedural_mesh_vertices_faces(kind, size):
    kind = str(kind)
    if kind == "flat_cylinder":
        return make_cylinder_mesh(size, segments=40)
    if kind == "l_shape":
        return make_l_block_mesh(size)
    if kind == "tetrahedron":
        return make_tetrahedron_mesh(size)
    if kind == "cube":
        return make_box_mesh(size)
    raise ValueError(f"Unknown procedural mesh kind: {kind}")
...
def run_falling(args, app, imports, out):
    ...
    cameras = {
        # Wide indoor-floor cameras: keep the full fall and the floor impact in frame.
        "overview": add_camera(Camera, "/World/Cameras/FallingOverview", "falling_overview", [2.55, -2.95, 1.70], [0.72, 0.00, 0.48], (args.width, args.height), 15.0),
        "closeup": add_camera(Camera, "/World/Cameras/FallingCloseup", "falling_closeup", [1.85, -1.95, 1.05], [0.72, 0.00, 0.34], (args.width, args.height), 22.0),
    }
    world.reset()
...
def run_rolling(args, app, imports, out):
    ...
    frames = max(int(args.frames), 420)
    args.frames = frames
    table_z = 0.0
    world = setup_falling_floor_world(World, FixedCuboid, PhysicsMaterial, UsdGeom, UsdLux, table_z)
    basic_catalog = [
        {"name": "builtin_cube", "category": "cube", "shape": "cube", "mass": 0.14, "size": [0.078, 0.078, 0.078], "color": [0.08, 0.52, 0.92], "source": "builtin_geometry", "visual_asset_path": "builtin://cube", "collision_asset_path": "builtin://cube"},
        {"name": "builtin_sphere", "category": "sphere", "shape": "sphere", "mass": 0.09, "size": [0.074, 0.074, 0.074], "color": [0.95, 0.30, 0.08], "source": "builtin_geometry", "visual_asset_path": "builtin://sphere", "collision_asset_path": "builtin://sphere"},
        {"name": "builtin_flat_cylinder_mesh", "category": "flat_cylinder", "shape": "procedural_mesh_convex_decomposition", "mesh_kind": "flat_cylinder", "mass": 0.12, "size": [0.125, 0.125, 0.032], "color": [0.12, 0.72, 0.24], "source": "builtin_geometry"},
        {"name": "builtin_l_shape_mesh", "category": "l_shape", "shape": "procedural_mesh_convex_decomposition", "mesh_kind": "l_shape", "mass": 0.16, "size": [0.125, 0.095, 0.052], "color": [0.94, 0.74, 0.12], "source": "builtin_geometry"},
        {"name": "builtin_tetrahedron_mesh", "category": "tetrahedron", "shape": "procedural_mesh_convex_decomposition", "mesh_kind": "tetrahedron", "mass": 0.11, "size": [0.105, 0.095, 0.086], "color": [0.52, 0.22, 0.86], "source": "builtin_geometry"},
        {"name": "builtin_rect_bar", "category": "bar", "shape": "box", "mass": 0.11, "size": [0.150, 0.038, 0.038], "color": [0.18, 0.58, 0.64], "source": "builtin_geometry", "visual_asset_path": "builtin://cuboid_bar", "collision_asset_path": "builtin://cuboid_bar"},
    ]
    robotwin_catalog = [
    ...
    elif args.rolling_object_set == "robotwin":
        catalog = robotwin_catalog
    else:
        catalog = robotwin_catalog + basic_catalog
    ...
    for i, sp in enumerate(specs):
        sp["x0"] = rng.uniform(-1.85, -0.95)
        sp["y0"] = rng.uniform(-1.02, 1.02)
        sp["initial_position_w"] = [sp["x0"], sp["y0"], 0.0]
        easy_motion = sp["category"] in {"box", "cube", "bar", "flat_cylinder", "l_shape", "tetrahedron", "plate", "hammer", "tray", "woodenmallet", "dumbbell", "brush", "remotecontrol", "stapler", "markpen"}
        if easy_motion:
            sp["static_friction"] = rng.uniform(0.07, 0.16)
            sp["dynamic_friction"] = rng.uniform(0.03, min(0.075, sp["static_friction"] - 0.018))
        else:
            sp["static_friction"] = rng.uniform(0.10, 0.22)
            sp["dynamic_friction"] = rng.uniform(0.05, min(0.11, sp["static_friction"] - 0.025))
        sp["restitution"] = rng.uniform(0.03, 0.16)

    def random_rolling_orientation(category):
        yaw = rng.uniform(-math.pi, math.pi)
        if category in {"hammer", "woodenmallet", "dumbbell", "brush", "remotecontrol", "stapler", "markpen", "bar", "l_shape", "tetrahedron"}:
            roll = rng.uniform(-1.25, 1.25)
            pitch = rng.uniform(-1.10, 1.10)
        elif category == "flat_cylinder":
            roll = rng.uniform(-0.72, 0.72)
            pitch = rng.uniform(-0.72, 0.72)
    ...
    def center_of_mass_offset(category, size):
        frac = {
            ...
            "tray": [0.00, 0.00, -0.04],
            "cube": [0.00, 0.00, 0.00],
            "flat_cylinder": [0.00, 0.00, -0.04],
            "l_shape": [0.16, -0.16, 0.03],
            "tetrahedron": [0.00, -0.05, -0.08],
    ...
    def inertia_diag_for(category, size, mass):
        ...
        scale = {
            ...
            "tray": [1.16, 1.02, 0.64],
            "cube": [1.00, 1.00, 1.00],
            "flat_cylinder": [1.24, 1.24, 0.48],
            "l_shape": [1.22, 0.96, 1.34],
            "tetrahedron": [0.82, 0.92, 0.78],
    ...
    for i, sp in enumerate(specs):
        ...
        elongated = sp["category"] in {"bar", "l_shape", "tetrahedron", "hammer", "woodenmallet", "dumbbell", "brush", "remotecontrol", "stapler", "markpen"}
        speed_xy = rng.uniform(1.85, 3.25 if elongated else 2.65)
        target_xy = np.array([rng.uniform(1.65, 3.10), rng.uniform(-0.82, 0.82)], dtype=np.float64)
        heading = math.atan2(float(target_xy[1] - sp["y0"]), float(target_xy[0] - sp["x0"])) + rng.uniform(-0.28, 0.28)
        spin_scale = 2.25 if elongated else 1.55
        lin = np.array([speed_xy * math.cos(heading), speed_xy * math.sin(heading), rng.uniform(-0.006, 0.014)])
        ang = np.array(
            [
                rng.uniform(-5.2, 5.2) * spin_scale,
                rng.uniform(-5.8, 5.8) * spin_scale,
                rng.uniform(-4.6, 4.6) * spin_scale,
            ]
        )
        q = random_rolling_orientation(sp["category"])
        sp["disturbances"] = []
        for _ in range(max(0, int(args.rolling_disturbances_per_object))):
            step = rng.randint(max(3, frames // 14), max(4, int(frames * 0.58)))
            duration_steps = rng.randint(4, 10)
            kick_heading = heading + rng.uniform(-0.72, 0.72)
            force_mag = rng.uniform(0.45, 1.85 if elongated else 1.25)
            force_w = np.array(
                [
                    force_mag * math.cos(kick_heading),
                    force_mag * math.sin(kick_heading),
                    rng.uniform(0.00, 0.18),
                ],
                dtype=np.float64,
            )
            contact_local_m = np.array(
                [
                    rng.uniform(-0.46, 0.46) * float(size[0]),
                    rng.uniform(-0.46, 0.46) * float(size[1]),
                    rng.uniform(-0.12, 0.36) * float(size[2]),
                ],
                dtype=np.float64,
            )
            offset_w = quat_rotate(q, contact_local_m)
            torque_w = np.cross(offset_w, force_w) + np.array(
                [
                    rng.uniform(-0.028, 0.028),
                    rng.uniform(-0.034, 0.034),
                    rng.uniform(-0.022, 0.022),
                ],
                dtype=np.float64,
            )
            impulse_dt = duration_steps / 120.0
            equiv_delta_v = force_w * impulse_dt / max(float(sp["mass"]), 1.0e-6)
            inertia_for_impulse = inertia_diag_for(sp["category"], size, float(sp["mass"]))
            equiv_delta_w = np.clip(torque_w * impulse_dt / np.maximum(inertia_for_impulse, 1.0e-6), -7.5, 7.5)
            sp["disturbances"].append(
                {
                    "step": int(step),
                    "duration_steps": int(duration_steps),
                    "kind": "force_torque_disturbance_with_eccentric_contact",
                    "force_w_N": [float(x) for x in force_w],
                    "torque_w_Nm": [float(x) for x in torque_w],
                    "contact_offset_local_m": [float(x) for x in contact_local_m],
                    "equivalent_delta_linear_velocity_w": [float(x) for x in equiv_delta_v],
                    "equivalent_delta_angular_velocity_w": [float(x) for x in equiv_delta_w],
                    "delta_linear_velocity_w": [float(x) for x in equiv_delta_v],
                    "delta_angular_velocity_w": [float(x) for x in equiv_delta_w],
                }
            )
        ...
        elif str(sp["shape"]).startswith("procedural_mesh"):
            vertices, faces = procedural_mesh_vertices_faces(sp.get("mesh_kind", sp["category"]), size)
            visual_vertices, visual_faces = vertices, faces
            sp["visual_asset_path"] = f"builtin://{sp.get('mesh_kind', sp['category'])}/visual_mesh"
            sp["collision_asset_path"] = f"builtin://{sp.get('mesh_kind', sp['category'])}/collision_mesh"
            sp["procedural_mesh_kind"] = str(sp.get("mesh_kind", sp["category"]))
            sp["collision_mesh_vertex_count"] = int(len(vertices))
            sp["collision_mesh_face_count"] = int(len(faces))
            sp["visual_mesh_vertex_count"] = int(len(visual_vertices))
            sp["visual_mesh_face_count"] = int(len(visual_faces))
        ...
        if str(sp["shape"]).startswith("robotwin_collision_mesh") or str(sp["shape"]).startswith("procedural_mesh"):
        ...
        objs.append(obj)
        visual_ops.append(None if (str(sp["shape"]).startswith("robotwin_collision_mesh") or str(sp["shape"]).startswith("procedural_mesh")) else make_follow_visual(world.stage, UsdGeom, f"/World/RollingVisual_{i}", sp["category"], size, sp["color"]))
    ...
    cameras = {
        "overview": add_camera(Camera, "/World/Cameras/RollingOverview", "rolling_overview", [4.85, -6.55, 3.15], [0.62, 0.00, 0.22], (args.width, args.height), 8.0),
        "closeup": add_camera(Camera, "/World/Cameras/RollingCloseup", "rolling_closeup", [2.75, -3.35, 1.32], [0.58, 0.00, 0.18], (args.width, args.height), 15.0),
        "left_side": add_camera(Camera, "/World/Cameras/RollingLeftSide", "rolling_left_side", [0.62, -5.10, 1.36], [0.62, 0.00, 0.18], (args.width, args.height), 10.0),
        "right_side": add_camera(Camera, "/World/Cameras/RollingRightSide", "rolling_right_side", [0.62, 5.10, 1.36], [0.62, 0.00, 0.18], (args.width, args.height), 10.0),
        "top": add_camera(Camera, "/World/Cameras/RollingTop", "rolling_top", [0.62, -0.35, 6.05], [0.62, 0.00, 0.02], (args.width, args.height), 8.5),
        "front": add_camera(Camera, "/World/Cameras/RollingFront", "rolling_front", [5.35, 0.00, 1.26], [0.62, 0.00, 0.18], (args.width, args.height), 10.0),
        "left_oblique": add_camera(Camera, "/World/Cameras/RollingLeftOblique", "rolling_left_oblique", [4.70, -4.55, 2.35], [0.62, 0.00, 0.20], (args.width, args.height), 8.5),
        "right_oblique": add_camera(Camera, "/World/Cameras/RollingRightOblique", "rolling_right_oblique", [4.70, 4.55, 2.35], [0.62, 0.00, 0.20], (args.width, args.height), 8.5),
    }
    world.reset()
    for cam in cameras.values():
        cam.initialize()
...
def collect_task(args, app, world, objects, cameras, out, task, table_z, meta, bottom_fn, sizes, shapes, masses, colors, visual_ops=None, disturbances=None):
    ...
    if support_surface == "indoor_floor":
        table_half_xy = np.array([7.00, 4.50], dtype=np.float32) * 0.5
    ...
    disturbance_force_w = np.zeros((frames, n, 3), np.float32)
    disturbance_torque_w = np.zeros((frames, n, 3), np.float32)
    disturbance_events = [[] for _ in range(frames)]
    active_disturbance_events = [[] for _ in range(frames)]
    if disturbances:
        dt = 1.0 / 120.0
        for obj_i, items in enumerate(disturbances):
            for item in items:
                ...
                if 0 <= step < frames:
                    ...
                    force_w = np.asarray(item.get("force_w_N", [0, 0, 0]), dtype=np.float32)
                    torque_w = np.asarray(item.get("torque_w_Nm", [0, 0, 0]), dtype=np.float32)
                    mass_i = max(float(masses[obj_i]), 1.0e-6)
                    inertia_i = np.asarray(meta[obj_i].get("inertia_diag_kg_m2", [1.0, 1.0, 1.0]), dtype=np.float32)
                    inertia_i = np.maximum(inertia_i, 1.0e-6)
                    item.setdefault("delta_linear_velocity_w", (force_w * (dt * duration_steps) / mass_i).astype(float).tolist())
                    item.setdefault("delta_angular_velocity_w", np.clip(torque_w * (dt * duration_steps) / inertia_i, -7.5, 7.5).astype(float).tolist())
                    ...
                    for active_step in range(step, min(frames, step + duration_steps)):
                        disturbance_force_w[active_step, obj_i] += force_w
                        disturbance_torque_w[active_step, obj_i] += torque_w
                        active_disturbance_events[active_step].append({"object_i": int(obj_i), **item, "active_step": int(active_step)})
    ...
    def clamp_demo1_motion():
        ...
        for obj in objects:
            try:
                v = np.asarray(obj.get_linear_velocity(), dtype=np.float64)
                w = np.asarray(obj.get_angular_velocity(), dtype=np.float64)
                v_norm = float(np.linalg.norm(v))
                w_norm = float(np.linalg.norm(w))
                if v_norm > max_lin:
                    obj.set_linear_velocity(v * (max_lin / max(v_norm, 1.0e-9)))
                if w_norm > max_ang:
                    obj.set_angular_velocity(w * (max_ang / max(w_norm, 1.0e-9)))
            except Exception:
                pass
    ...
    for t in range(frames):
        if disturbance_events[t]:
            for item in disturbance_events[t]:
                events.append({"step": t, "type": "random_disturbance_force_torque", **item})
        if active_disturbance_events[t]:
            dt = 1.0 / 120.0
            for item in active_disturbance_events[t]:
                i = int(item["object_i"])
                force_w = np.asarray(item.get("force_w_N", [0, 0, 0]), dtype=np.float64)
                torque_w = np.asarray(item.get("torque_w_Nm", [0, 0, 0]), dtype=np.float64)
                used_runtime_force_api = False
                try:
                    view = getattr(objects[i], "_rigid_prim_view", None) or getattr(objects[i], "_view", None)
                    method = getattr(view, "apply_forces_and_torques_at_pos", None)
                    if callable(method):
                        method(forces=np.asarray([force_w], dtype=np.float32), torques=np.asarray([torque_w], dtype=np.float32), positions=None, is_global=True)
                        used_runtime_force_api = True
                except Exception:
                    used_runtime_force_api = False
                if not used_runtime_force_api:
                    try:
                        mass_i = max(float(masses[i]), 1.0e-6)
                        inertia_i = np.asarray(meta[i].get("inertia_diag_kg_m2", [1.0, 1.0, 1.0]), dtype=np.float64)
                        inertia_i = np.maximum(inertia_i, 1.0e-6)
                        dv = np.clip(force_w * dt / mass_i, -0.45, 0.45)
                        dw = np.clip(torque_w * dt / inertia_i, -4.0, 4.0)
                        objects[i].set_linear_velocity(objects[i].get_linear_velocity() + dv)
                        objects[i].set_angular_velocity(objects[i].get_angular_velocity() + dw)
                    except Exception:
                        pass
        clamp_demo1_motion()
    ...
    dataset = {
        ...
        "disturbance_force_w_N": disturbance_force_w,
        "disturbance_torque_w_Nm": disturbance_torque_w,
    ...
    manifest = {
        ...
        "builtin_geometry_visual_and_collision_count": len([x for x in meta if str(x.get("source", "")) == "builtin_geometry" and x.get("visual_asset_path") and x.get("collision_asset_path")]),
        ...
        "max_xy_displacement_m": float(np.linalg.norm(pos[:, :, :2] - pos[0:1, :, :2], axis=-1).max()),
        "max_force_disturbance_N": float(np.linalg.norm(disturbance_force_w, axis=-1).max()),
        "max_torque_disturbance_Nm": float(np.linalg.norm(disturbance_torque_w, axis=-1).max()),
        "demo1_v5_notes": "Single object on textured indoor-room floor; combined RobotWin meshes and builtin geometry objects (cube, sphere, flat cylinder, L block, tetrahedron) use matching visual/collision geometry; lower friction, longer simulation, eccentric force/torque disturbances, overview/closeup/left_side/right_side/top/front/left_oblique/right_oblique RTX cameras with 2D boxes and pixel masks overlaid in every video.",
    }
...
def main():
    ...
    ap.add_argument("--rolling-object-set", choices=["basic", "robotwin", "combined", "extended"], default="combined")
